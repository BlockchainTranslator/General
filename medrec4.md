...
隐私方面：使用区块链技术产生了几个缺陷。交易的匿名性允许对数据取证，或者从频率分析中推断治疗模式。不泄露姓名的话，人们也能够从网络流量分析中推断出某个实体一再地与另一个实体有所联系。一个可能的解决方案是使区块链成为一个“被许可的”结构，其中，只有前期被同意的、白名单上的节点被允许对账簿进行读取操作。这将防止恶意节点从区块链中的看病记录里抽取出某些以频率为参考的数据。另外，加密可以被引入链外数据同步过程来保护偶然的或者恶意的内容访问。
